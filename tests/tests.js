...
describe("Tests", function () {
    ...
    it("performance benchmark", function () {
        ...
        // toBinary
        bench(`toBinary v4 (${iterations}x)`, () => {
            for (const ip of v4Ips) ipUtils.toBinary(ip);
        });
        bench(`toBinary v6 (${iterations}x)`, () => {
            for (const ip of v6Ips) ipUtils.toBinary(ip);
        });

        // expandIP
        bench(`expandIP v4 (${iterations}x)`, () => {
            for (const ip of v4Ips) ipUtils.expandIP(ip);
        });
        bench(`expandIP v6 (${iterations}x)`, () => {
            for (const ip of v6Ips) ipUtils.expandIP(ip);
        });

        // isValidPrefix
        bench(`isValidPrefix v4 (${iterations}x)`, () => {
            for (const p of v4Prefixes) ipUtils.isValidPrefix(p);
        });
        bench(`isValidPrefix v6 (${iterations}x)`, () => {
            for (const p of v6Prefixes) ipUtils.isValidPrefix(p);
        });

        // applyNetmask
        bench(`applyNetmask v4 (${iterations}x)`, () => {
            for (const p of v4Prefixes) ipUtils.applyNetmask(p);
        });
        bench(`applyNetmask v6 (${iterations}x)`, () => {
            for (const p of v6Prefixes) ipUtils.applyNetmask(p);
        });

        // isSubnet
        bench(`isSubnet v4 (${iterations}x)`, () => {
            for (const p of v4Prefixes) ipUtils.isSubnet("10.0.0.0/8", p);
        });
        bench(`isSubnet v6 (${iterations}x)`, () => {
            for (const p of v6Prefixes) ipUtils.isSubnet("2001:db8::/32", p);
        });

        // isEqualPrefix
        bench(`isEqualPrefix v4 (${iterations}x)`, () => {
            for (const p of v4Prefixes) ipUtils.isEqualPrefix(p, p);
        });
        bench(`isEqualPrefix v6 (${iterations}x)`, () => {
            for (const p of v6Prefixes) ipUtils.isEqualPrefix(p, p);
        });
